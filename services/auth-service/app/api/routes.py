...
async def google_login(payload: GoogleLoginRequest, db: AsyncSession = Depends(get_db)):
    return await AuthCommandService(db).google_login(payload)
...
async def forgot_password(payload: ForgotPasswordRequest, db: AsyncSession = Depends(get_db)):
    return await VerificationCommandService(db).forgot_password(payload.email)

@router.post("/password/reset", response_model=AuthActionResponse)
async def reset_password(payload: ResetPasswordRequest, db: AsyncSession = Depends(get_db)):
    return await VerificationCommandService(db).reset_password(payload)
...
async def request_email_verification(payload: EmailVerifyRequest, db: AsyncSession = Depends(get_db)):
    return await VerificationCommandService(db).request_email_verification(payload)
...
async def confirm_email_verification(payload: EmailVerifyConfirmRequest, db: AsyncSession = Depends(get_db)):
    return await VerificationCommandService(db).confirm_email_verification(payload)
